...
local function cupid_load(args)
	...
	if use then
		setmetatable(wrapped_love, {__index = love, __newindex = protector})
		_love = love
		love = wrapped_love
		for k,v in pairs(protected_funcs) do
			_love[v] = function(...)
...
mods.physics = function() return {
	...
	["pre-load"] = function(self)
		...
		local wrapped_physics = {}
		wrapped_physics.newWorld = function(...)
			local out = {physics.newWorld(...)}
			self.world = out[1]
			return unpack(out)
		end
		setmetatable(wrapped_physics, {__index=physics})
		rawset(wrapped_love, "physics", wrapped_physics)
	end,
	["post-draw"] = function(self)
