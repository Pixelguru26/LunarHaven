...
function UI.update(dt)
	...
	if controls.isDown('place') and not Vec(love.mouse.getX(),love.mouse.getY()):isWithinRec(Rec(gw()-ui.hotBarWidth*gw(),gh()*ui.hotBarMargin,UI.hotBarCanv:getWidth(),UI.hotBarCanv:getHeight())) and not game.system.isUIEnabled("pixelEditor") then
		if last then
...
function UI.mousepressed(x,y,b)
	...
	if pos:isWithinRec(Rec(gw()-ui.hotBarWidth*gw(),gh()*ui.hotBarMargin,UI.hotBarCanv:getWidth(),UI.hotBarCanv:getHeight())) then
		...
		if controls.isIn("place",b) and not game.system.isUIEnabled("pixelEditor") then
			UI.placeBlock(UI.hotbar.selIndex,ox,oy)
		elseif controls.isIn("scrlUp",b) and not game.system.isUIEnabled("pixelEditor") then
			UI.hotbar.selIndex=math.wrap(UI.hotbar.selIndex-1,1,10)
		elseif controls.isIn("scrlDn",b) and not game.system.isUIEnabled("pixelEditor") then
			UI.hotbar.selIndex=math.wrap(UI.hotbar.selIndex+1,1,10)
		end
	end
end
